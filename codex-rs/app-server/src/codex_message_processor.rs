...
use codex_app_server_protocol::TurnInjectParams;
use codex_app_server_protocol::TurnInjectResponse;
...
impl CodexMessageProcessor {
    ...
    pub async fn process_request(
        ...
        match request {
            ...
            ClientRequest::TurnInject { request_id, params } => {
                self.turn_inject(to_connection_request_id(request_id), params)
                    .await;
            }
    ...
    async fn turn_inject(&self, request_id: ConnectionRequestId, params: TurnInjectParams) {
        let TurnInjectParams { thread_id, message } = params;
        let (_, thread) = match self.load_thread(&thread_id).await {
            Ok(v) => v,
            Err(error) => {
                self.outgoing.send_error(request_id, error).await;
        ...
        thread
            .inject_developer_message_without_turn(message)
            .await;
        self.outgoing
            .send_response(request_id, TurnInjectResponse {})
            .await;
    }
